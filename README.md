...
```
├── src/                          # Исходники
│   ├── js                        # Скрипты | СЮДА ВПЕНДЮРИВАЕМ ВСЕ ЧТО ПИШЕМ
│   │   └── custom                # кастомные модули | УДАЛИТЬ НЕ ИСПОЛЬЗУЕМЫЕ
...
`npm run deploy-in-gh` - запускает деплой папки app в ветку gh-pages в репазитории github, одновременно разорхивируя ее.
...
Для сборки JS-кода используется минификтор JavaScript кода, по итогу в деплой версии у вас сохраняются вся структура которая была внесена вами при написании проекта, все файлы сохраняют свои названия, пути и минифицируются. 

Не забывайте удалять не используемые компоненты из дериктории `custom`, добавлять или удалять записи из `head.html`

## Работа со шрифтами

Т.к. автор не поддерживает IE11 (и вообще зачем он нужен), в сборке реализована поддержка только формата __woff2__ (это значит, что в миксине подключения шрифтов используется только данный формат).
...
1. Зайти в файл `js/custom/burger.js` и раскомментировать файл
...
4. Зайти в файл `js/custom/graph-modal.js` и раскомментировать.
...
1. Раскомментируйте файл `js/custom/header_height.js`
...
3. Подключить сам свайпер в html код используя `cdn` или `bundle file` и использовать его, следуя документации.

### Плавный скролл к якорям

Вы можете сделать плавный скролл к якорям, который работает даже в Safari, с помощью плагина. Как это использовать:

1. Подключить код библиотеки smooth-scroll.js (код в файле `js/custom/smooth-scroll.js`) и инициализировать его, передав селектор всех якорных ссылок.
